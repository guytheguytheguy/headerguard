...
- Bumped `next` and `eslint-config-next` to `15.5.21` (latest patch in the 15.x line — matches the target other portfolio projects already moved to, skips the incomplete intermediate 15.5.17 patch per prior portfolio finding, and avoids the untested 16.x major jump). `npm install` took ~26 min due to heavy concurrent `node.exe` load from other portfolio agents on this shared machine (26 processes observed), not a real problem — one earlier install attempt hit a Windows `ENOTEMPTY` file-lock error mid-cleanup on `node_modules/next`, worked around by manually removing the stale directory and retrying clean.
- Re-verified no regression on the new version: `vitest run` **74/74 pass**, `playwright test` **23/23 pass**, `next build` clean (12 routes, TypeScript clean, confirmed running `Next.js 15.5.21` in the build log).
- `npm audit` still reports 12 issues (1 critical, 8 high) but every one is in a devDependency (vite, vitest, esbuild, ws, js-yaml, etc. — build/test tooling, not shipped to production) — left for a future pass, out of scope for this targeted security fix.
- Committed (`d710956`) and pushed local `main` to a new backup branch `backup-local-main-20260722` (origin/main push still rejected — non-fast-forward, git divergence blocker below still open, 6th consecutive daily run). Deployed the tested build directly via `vercel --prod` (`dpl_CvKzCCEWE4HAEBirduqNjBrxVpiw`, READY, aliased to `headerguard.veridux.ai`) since the git→Vercel auto-deploy pipeline remains broken by that same divergence.
- Live-verified post-deploy: homepage/pricing 200, SSRF guard still blocks `169.254.169.254` (400), a normal scan of `example.com` still succeeds (200), `POST /api/checkout` still `503 {"error":"Billing not configured"}` (unchanged Stripe env var blocker, unrelated to this fix). `headerguard.com` apex DNS re-confirmed still parked (`185.212.70.120`, unrelated to Vercel) — unchanged.
- Standing blockers unchanged, no action taken: (1) Stripe env vars still unset in Vercel prod (`vercel env ls production` shows only 3 Supabase-related vars, zero Stripe vars) — revenue blocker, human required. (2) Git divergence — `git merge-base main origin/main` still empty, `origin/main` (`242be1d`) still only the 2026-06-25 initial commit — human reconciliation decision still needed, did not force-push.
...
- Also found uncommitted, working-but-uncommitted code already sitting in the working tree from a previous session: `src/app/api/webhooks/stripe/route.ts` had been rewritten to write to `subscriptions` (matching the live schema exactly) instead of the old `profiles` table (which was never wired up either). Verified it against the live schema, ran it (6 new tests, all passing), and committed it (`8d1c367`) -- it was real, correct, finished work that just never got committed.
- New `src/lib/scan-limit.ts` (commit `9cbf0a5`): `resolveScanIdentity` verifies a client-supplied Supabase access token against Auth (never trusts a raw client-supplied user id -- `/api/checkout` already had that trust gap for the userId it accepts, didn't want to add a second one here); `checkScanLimit` looks up `subscriptions.plan` for an active/trialing Pro subscriber to bypass the limit entirely, otherwise counts real rows in `scans` from the last 24h by `user_id` (signed in) or `ip_address` (anonymous) and blocks at 3/day; `recordScan` persists every completed scan. All three fail open (allow the scan) if Supabase is unreachable -- a rate-limit-tracking outage should not take down the core scanning product. This is a real, persistent, cross-serverless-instance counter, not an in-memory one (which would silently reset per Vercel instance and enforce nothing).
- `src/app/api/scan/route.ts`: wired in -- checks the limit before scanning, returns `429` with an upgrade message when exceeded, records the scan (awaited, not fire-and-forget, since a frozen/killed serverless function after response-send would silently drop the write and make the limit trivially bypassable).
- `src/components/scan-form.tsx`: now passes the caller's Supabase access token (if signed in) so Pro users get the bypass; surfaces the 429 with an upgrade CTA message. Also corrected the "No data is stored" caption, which became false the moment scan metadata started being persisted for rate-limiting -- it now discloses that URL/grade/score are kept for that purpose (the separate FAQ claim "no data from your site is stored" is about page content/forms, remains accurate, left as-is).
- New tests: `src/lib/scan-limit.test.ts` (13 cases: IP extraction, token verification/rejection, Pro bypass, free-tier counting at/under/over the limit, fail-open with no identity, scan persistence shape, insert-failure resilience). Full unit suite: **74/74 pass** (was 55: +6 webhook, +13 scan-limit). `tsc --noEmit`: clean. `npm run build`: started but did not finish within this session's time budget -- the shared machine had 68-72 concurrent `node.exe` processes from other same-day portfolio agents, and the build log stayed at "Creating an optimized production build..." for 15+ minutes with active `.next/cache` writes (not hung, just severely CPU-starved). Typecheck + full test suite both clean is a strong signal but this is a real gap versus a confirmed production build -- flagging for the next run to re-verify `npm run build` completes clean once machine load drops.
- Re-verified all three standing human-only blockers, all still open, no action taken: (1) `POST /api/checkout` -> `503 {"error":"Billing not configured"}`, live-curl-confirmed. (2) Git divergence unchanged -- `origin/main` (`242be1d`) still only the unrelated 2026-06-25 initial commit, `git merge-base main origin/main` still empty; did not force-push, backed up to new branch `backup-local-main-20260720` (pushed) alongside the existing `-20260713`/`-20260716`/`-20260717` branches. (3) `headerguard.com` -- a local ISP-level content filter was intercepting plain DNS lookups mid-run (resolving to a `securingsam.com` blockpage IP, unrelated to this domain); authoritative DNS-over-HTTPS (Cloudflare + Google, bypassing that interception) confirms the apex is actually delegated to `ns1/ns2.dns-parking.com` at `185.212.70.120` -- a registrar parking page. Confirms it is not, and was never, pointed at Vercel; `headerguard.veridux.ai` remains the only live domain (HTTP 200).
